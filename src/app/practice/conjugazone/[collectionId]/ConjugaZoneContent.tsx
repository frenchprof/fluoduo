...
function splitSentence(fr: string): { subject: string; predicate: string } {
  const idx = fr.indexOf(" ");
  if (idx === -1) return { subject: fr, predicate: "" };
  return { subject: fr.slice(0, idx), predicate: fr.slice(idx + 1) };
}

export default function ConjugaZoneContent({ collectionId }: { collectionId: string }) {
  const deck = CURATED.find((c) => c.id === collectionId);
  const tabs = useMemo(() => (deck ? withActive(deckActivityTabs(deck.id), "conjugazone") : []), [deck]);
  ...
  const { subject, predicate } = item ? splitSentence(item.fr) : { subject: "", predicate: "" };
  const isRight = result === "perfect" || result === "good";

  function check() {
    if (result !== null || !item) return;
    const g = grade(value, predicate);
    setResult(g);
  ...
  return (
    ...
    >
      <div className="mx-auto max-w-lg px-4 py-6">
        <h1 className="fluo-serif text-2xl font-black text-[color:var(--fluo-ink)]">🎯 ConjugaZone</h1>
        ...
        {done ? (
          ...
          <div className="rounded-2xl border-2 bg-[var(--fluo-card)] p-4" style={{ borderColor: "var(--fluo-line)" }}>
            <p className="mt-1 text-xl font-black text-[color:var(--fluo-ink)]">
              <span className="text-[color:var(--cahier-ink-soft)] font-medium">{subject} </span>
              <span>{item.en}</span>
            </p>
            ...
            <form onSubmit={(e) => { e.preventDefault(); result === null ? check() : next(); }} className="mt-4">
              <input
                ref={inputRef}
                lang="fr"
                value={result === null ? value : predicate}
                onChange={(e) => setValue(e.target.value)}
                disabled={result !== null}
                placeholder={`commence par « ${predicate[0] ?? "?"} »…`}
