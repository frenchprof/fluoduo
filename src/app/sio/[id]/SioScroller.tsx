    ...
    <SnapFeed startAt={startAt} onIndex={onIndex} sectionClassName="justify-start px-1 pb-4 pt-6">
      {SIOS.map((s) => (
        <div key={s.id} id={`goal-${s.id}`} data-sio={s.id}>
          {/* THE ICONS ARE BACK INSIDE THE BOX (Dan, 2026-09-11, drawing the
              icons into the card and an arrow up to them from where they were).
              This reverses his 7 Sep "the icons can just be by themselves below
              that", and the screenshot is why: the words box is a measured
              21rem that most goals do not fill, so the card ended as a tall
              empty rectangle with its five doors stranded on the paper below
              it. Inside, that empty space is what holds them at one height on
              all fifty — the thing the fixed box was for in the first place. */}
