 ...
 *
 * The two variable fields cycle and settle, then the sentence is reduced
 * three times and merged once. The stages, the beats and the layout
 * arithmetic all live in `src/lib/fluolingoOrigin.ts`; this file is the
 * renderer and knows only about pixels.
 *
 ...
 *
 * The base size is fitted to the WIDEST sentence any pair of cycling words
 * can make, not to the settled one, so nothing is ever cropped and nothing
 * resizes while the fields are still spinning. After that the size only goes
 * up: `growth` per transformation, compounding.
 *
 * One rAF loop writes transforms straight to the nodes — no React state per
 * frame, so ~50 glyphs at 60fps cost one style write each.
 */
...
const SURVIVOR_TOKEN: Record<number, string> = {
  0: "--fluo-origin-flu",
  2: "--fluo-origin-o",
  4: "--fluo-origin-lin",
  5: "--fluo-origin-go",
...
const CHIP_TOKEN: Record<number, string> = {
...
export type FluolingoOriginProps = {
  className?: string;
  ...
  useEffect(() => {
    ...
    function draw(t: number) {
      ...
      A.L.items.forEach((a) => {
        ...
        // color-mix, not arithmetic on a parsed hex: the ink token is
        // authored in lab(), and the browser is the only thing that reads
        // every colour syntax the stylesheet is allowed to use.
