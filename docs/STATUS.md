...
| 8 | **Retention work approved 21 Aug, not yet built.** PWA manifest; the six new `fluolingo:reward` events; floating +XP with the multiplier paying out; session receipts; weekly leaderboard + neighbours. Hero-mark colouring is re-scoped to **two** marks after round 12 (course + streak; level/XP moved to /moi and /profil). Still needing Dan's pedagogical call: the daily goal's size, variable reward, the streak freeze, the gem locker. **The ethics constraints in DOPAMINE_REVIEW are binding** — no loss-framing, no guilt, never punish absence. | 6 | agent |
