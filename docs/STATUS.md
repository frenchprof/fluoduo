...
| 8 | **Dopamine / retention review — `docs/DOPAMINE_REVIEW.md` (21 Aug, agent).** The question behind the colour brief: what makes a learner open it tomorrow. The economy in `economy.ts` is sound and stays. The gaps are between it and the learner — **no PWA manifest, no service worker, no notification of any kind** (so no way back in on a phone); no daily goal, so no completion moment; the streak increments silently and the ×1.5/×2 multiplier upgrade is invisible; XP is never shown at the moment it is earned; `fluolingo:reward` fires on 2 of 8 trackable moments; every reward is fixed-rate; the gem locker totals 150 gems. 10 ranked proposals, items 1-5 ≈ a week. Includes an ethics line (no loss-framing, never punish absence) since this is a graded course. **Proposals only — pedagogical calls, needs Dan.** | 1 | **Dan to decide, then agent** |
