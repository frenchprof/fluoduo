...
| 9 | **Retention work approved 21 Aug, not yet built.** PWA manifest; six new `fluolingo:reward` events; floating +XP; session receipts; weekly leaderboard + neighbours; the two hero-mark swaps. Still needing Dan's pedagogical call: daily-goal size, variable reward, streak freeze, gem locker. **Ethics constraints are binding.** New chrome goes in `.cahier-topslot`, never the icon strip — see row 8. | 6 | agent |
