...
- Items **1–9** = teachable / finishable now; **10–18** = harden later.
...
| **SUP-CAL** | Calendar supplements sit under **item 10 Optional**, **off the Continue spine**. Core SIOs **001–050** stay intact. A unit can complete without opening a calendar. |
...
| **9** | **FR (Enchantée, Libéria, Flip P1s)** | SIO-010 keyed *Enchanté* after Léa; expert list had *Liberia*. Flip It P1s in #154. | Merge **#156**. Then a follow-up PR for #154 Flip It P1s: `sappeler-05` / `-06`; salutations *Enchantée* card; *Bonne nuit* bedtime-only; `core-nouns-06` *salle de classe*. Hold Bélarus / Birmanie / Cap-Vert / Centrafrique. | Léa → **Enchantée** only (Marc → Enchanté). Role cue `Léa · she` visible **before** the guess. EN WHY on the wrong form. **Libéria** / `LIBÉRIA` on the expert tile. Flip P1s FR QA green. `verify91-enchantee-liberia` green. | FR + Pedagogy → Coordinator merge, then Claude(impl) for P1s |
| **10** | **Curriculum (v9 + SUP-CAL)** | Core is the v9 50-SIO spine. Directions stay off the imperative (except SIO-009 consignes). Calendar is optional extra. | Reconcile numbering to `docs/handoff/LAF1201_SIOs_Flashcards_v9.csv`. Guard-rail: SIO-036 / SIO-040 use *il faut* / *on* / *c’est* — *Tournez / Allez / Prenez* only as distractors. Add **SUP-CAL-01 / 02 / 03** as Optional chips **off Continue**. Continue never requires a calendar stop. | Core **50** complete without opening SUP-CAL. Optional chip visible, not on the spine. Unit complete **without calendar**. Directions bank has no keyed imperative. | Pedagogy + Claude(impl) |
